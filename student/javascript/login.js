$(document).ready(function () {
  ...
  $('#loginform').bind("keypress", function (e) {
    if (e.keyCode == 13) {
      e.preventDefault();
      console.log("$2y$10$GG9eojd14GsKHtSP3CFJH.YfsPcr.nH9g1mqe2YB30cEcnGn8a0LK");
      return false;
    }
  });
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data == 1) {
          ...
          $("#loginemail").attr("disabled", true);
          $("#loginform").append(" <div class='form-control'> <label for='password'>Enter New Login Password</label> <input type='password'  placeholder='Enter Password' id='password'/> <br> <label for='confirm_password'>Confirm Login Password</label> <input type='password'  placeholder='Confirm Password' id='confirm_password'/> </div> <button id='create_password' class='inner_button'>Create Password</button>");
  ...
  $(document).on("click", "#create_password", function (e) {
    ...
    var mailformat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    if (password === confirm_password && mailformat.test(email) && password.length > 7) {
      $.ajax({
      ...
      swal("ohoho!",
        "Password does not match! enter correct password in both field's! or Password Should be More then 8 digit's",
        "error");
    }
  });
