...
class sendassign extends db_connection
{
...

    {
        ...
        if ($countteacher > 0 && $countsemester > 0 && $countsubject > 0) {
            $sql = $this->conn->prepare("SELECT * FROM `assignedsubject` WHERE semesterid = ? && subjectid = ?  && assignedstatus = 'active'");
            ...
            if ($count > 0) {
                ...
                $disabled = "disabled";
                $sql = $this->conn->prepare("SELECT * FROM `assignedsubject` WHERE semesterid = ? && subjectid = ? && teacherid = ? && assignedstatus = ? ");
                $sql->bindParam(1, $getsemesterid);
                $sql->bindParam(2, $getsubjectid);
                $sql->bindParam(3, $getteacherid);
                $sql->bindParam(4, $disabled);
                $sql->execute();
                $countin = $sql->rowCount();
                if ($countin > 0) {
                    echo 9;
                } else {
