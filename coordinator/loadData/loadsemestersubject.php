...
class loadsemestersubject extends db_connection
{
    ...
    function __construct($getsemesterid, $getcoodinatorid)
    {
        ...
        $getBatchid->bindParam(1, $getsemesterid);
        $getBatchid->execute();
        if ($getBatchid->rowCount() > 0) {
            foreach ($getBatchid->fetchAll(PDO::FETCH_ASSOC) as $bt) {
                $batchids = $bt['batchid'];
                break;
            }
        ...
        <select id='assignedselect' placeholder='Pick a Subject...' class='form-select' aria-label='Default select example'><option value='0'>Open this to Select a Subject</option>";
            ...
            $sql = $this->conn->prepare("SELECT * FROM `subject` INNER join `assignedsyllabus` ON subject.subjectid = assignedsyllabus.subjectid WHERE assignedsyllabus.batchid = ?");
            $sql->bindParam(1, $batchids);
            $sql->execute();
...
";
        ...
        <td style='color:green;' class='table-primary'>";
            ...
            $output .= "Semester does not exist";
        }
